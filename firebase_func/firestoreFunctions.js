...
export async function getCarportData(onwer_id) {
  var data = {};
  await db.collection('carports').doc(onwer_id).get().then(
    function(doc) {
      if (doc.exists) {
        data = doc.data();
      } else {
...
export async function getOwnedCarports(owner_id){
  var ownedCarports = await db.collection('carports').where('owner_id', '==', owner_id).get()
    .then(querySnapshot => {
      var carportArray = [];
...
export async function updateCarportData(port_id, data){
  ...
  var returnVal =
    await db.collection('carports').doc(port_id)
      .update(newdata)
      .then(res => {
        return res;
      });
...
export async function queryNearbyGeocode(){
  ...
  geoCollection.doc('test0').get().then(res => console.log(res.data()));
  const query = geoCollection.near({ center: new firebase.firestore.GeoPoint(40.6568523,-111.956537), radius: 100000 });
  query.get().then((value) => {
    console.log('docs',value); // All docs returned by GeoQuery
  });
}

export async function queryNearbyCoordinates(min_lat, max_lat, min_lng, max_lng){
  let latBounds = await db.collection('carports')
    ...
    .get().then(
      ...
      err => console.error('error getting documents',err)
    );

  let lngBounds = await db.collection('carports')
...
export async function checkCarportAvailablity(carport) {
  ...
  if (!carport.allday){
    if (moment().isBetween(moment(carport.start, 'HH:mm'), moment(carport.end, 'HH:mm'))){
...
export async function createReservation(user_id, user_data, start, end, vehicle_id, vehicle_data, carport_id, carport_data, price, hours){
  ...
  const docRef =
    db.collection('carports').doc(carport_id)
      .collection('reservations').doc(rootRef.id);

  batch.set(rootRef, newReservation);
  batch.set(docRef, newReservation);

  let batchResult = await batch.commit().then(res => {
...
export async function addSaveLocation(user_id, title, formatted_address, place_id, lat, lng) {
  ...
  returnValue = await userDoc.set(newLocationData)
    .then(res => {
      console.log(res);
      return true;
    }).catch(err => console.error(err));
...
export async function getSavedLocations(user_id){
  var docRef =
    await db.collection('users').doc(user_id).collection('saved_locations')
    .where('user_id', '==', user_id)
    .limit(10)
    .get()
    .then((snapshot) => {
...
export async function addVehicle(owner_id, updates){
  ...
  var batchResult = await batch.commit().then( res => {
